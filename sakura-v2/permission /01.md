...
So the **order** for the user is: (1) request-level fields → submit creates request + headers; (2) if the flow includes RLS details in the same screen or a follow-up, user fills dimension fields per workspace → backend writes RLSPermissions + the correct RLSPermission*Details table.
