...
Optional: **RequestCode** — if user can set it: **Direct** (e.g. `REQCD10001`); otherwise system-generated.
...
- **Direct:** User types freely (e.g. RequestReason, RequestCode, AdditionalDetailsJSON).
...
- **Validation:** WorkspaceId exists (any valid workspace: CDI, AMER, WFI, GI, DFI, EMEA, etc.); required fields not empty; RequestCode unique if provided.
...
- Form: **Workspace** (dropdown must include **all** workspaces—see `Excel/` for the list), RequestedFor, RequestedBy, LMApprover, RequestReason. Submit → `createRequest(...)`. Success: toast + go to list or refresh list. Error: show message (e.g. duplicate RequestCode).
