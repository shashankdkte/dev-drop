...
3. **We use tabular data entry, not a wizard.**  
   All user values are entered on **one form, one screen**. No step-by-step wizard, no moving from page to page or grid to grid. One submit sends everything; the backend writes to PermissionRequests, PermissionHeaders, and (when applicable) OLSPermissions, RLSPermissions, and the workspace’s RLSPermission*Details in one go.
...
#### dbo.OLSPermissions (7 columns populated; Id is IDENTITY)

Used when the request includes **OLS** (Object-Level Security): one row per OLS permission, linked to a PermissionHeader with PermissionType = 0 (OLS).

| # | Column | DB type | How populated |
|---|--------|---------|----------------|
| 1 | PermissionHeaderId | INT | **From parent** (Id of new OLS PermissionHeaders row, i.e. header with PermissionType=0) |
| 2 | OLSItemType | INT | **User** (dropdown: 0 = Workspace Standalone Report / SAR, 1 = Audience) |
| 3 | OLSItemId | INT | **Lookup** (Id from WorkspaceReports when OLSItemType=0, or from AppAudiences when OLSItemType=1; user selects report or audience, backend resolves Id) |
| 4 | AdditionalDetailsJSON | NVARCHAR(2048) | **User** or {} (optional) |
| 5 | CreatedAt | DATETIME2(0) | **System** |
| 6 | CreatedBy | NVARCHAR(255) | **System** (current user) |
| 7 | UpdatedAt, UpdatedBy | DATETIME2(0)/NVARCHAR(255) | **System** |

**Note:** A request can have both OLS and RLS: two PermissionHeaders (one PermissionType=0, one PermissionType=1), then one row in OLSPermissions and one in RLSPermissions (plus the workspace’s RLSPermission*Details). If the request is OLS-only, only the OLS header and OLSPermissions are created.
...
- **Single flow:** For each workspace, all data for one permission request (request-level + RLS dimensions, and OLS if applicable) is taken in **one flow**; one submit populates PermissionRequests, PermissionHeaders, and—when applicable—**OLSPermissions** (for OLS) and/or **RLSPermissions** + the workspace’s **RLSPermission*Details** (for RLS).
- **Request (all workspaces):** User provides Workspace, RequestedFor, RequestedBy, LMApprover, RequestReason. System generates RequestCode. Backend writes **PermissionRequests** and **PermissionHeaders** (one or two headers: OLS and/or RLS).
- **OLS (if applicable):** User provides OLS item type (report vs audience) and selects report or audience; backend writes **OLSPermissions** (PermissionHeaderId, OLSItemType, OLSItemId, AdditionalDetailsJSON) linked to the OLS PermissionHeader. See **Column reference: dbo.OLSPermissions** for column count, types, and how populated.
...
- **View `romv.PermissionRequests`**  
  ...
  **Why:** So the list and detail UI can **show** RequestCode and LMApprover for review and approval. The view is read-only; data entry writes to `dbo.PermissionRequests`. This change is only for display/review, not for writing data.
...
### DB changes
- Change one **view** only: `romv.PermissionRequests`.
- Add two columns to that view: **RequestCode** and **LMApprover** (they already exist in the table; the view will now return them so the app can show them).
- Do **not** add new tables. Do **not** change the table definitions or the other view.

### Backend changes
- **Save to DB:** Add or use code that can **insert** into the real tables (`PermissionRequests`, `PermissionHeaders`, and when needed `OLSPermissions`, `RLSPermissions`, and the workspace Details table). That usually means entities or repos that write to `dbo`, not only read from the view.
- **Create request API:** One new API (e.g. POST) that takes Workspace, RequestedFor, RequestedBy, LMApprover, RequestReason; generates RequestCode; inserts one request row and header row(s); returns id and RequestCode. Controller calls a service; service does the insert.
- **Approve / Reject API:** Two new APIs (or one with an action) so the app can approve or reject one header. Controller receives the request id and header id; service loads the header, checks the user is allowed, then updates the header and request status.
- **List API:** When returning the list of requests, include **RequestCode** and **LMApprover** in the response (from the view, once the view is updated).

### Frontend changes
- **New screen (tabular entry, not wizard):** One form on one screen. The user picks Workspace, enters RequestedFor, RequestedBy, LMApprover, RequestReason, and—on the same form—any OLS fields (if needed) and all RLS dimension fields for the chosen workspace. **One submit** sends everything; no moving from page to page or grid to grid.
- **Details modal:** On the existing request-details screen, add **Approve** and **Reject** buttons when the header is pending and the user is allowed. Approve calls the approve API; Reject asks for a reason and calls the reject API. Refresh the data after.
- **List:** Optionally show RequestCode and LMApprover in the list table (once the backend returns them).
- **Config:** Add the new API URLs and the create/approve/reject calls in your API config and services.

### Tabular entry (not wizard): how the form connects to the tables

We use **tabular data entry**, **not a wizard flow**.

- **One screen, one form.** The user does **not** move from page to page or from grid to grid. All user-provided values are entered on that single form.
- **What’s on the form:** Request fields (Workspace, RequestedFor, RequestedBy, LMApprover, RequestReason). Depending on workspace and request type: OLS fields (e.g. OLS item type, report or audience) and all RLS dimension fields for that workspace (e.g. EntityKey, EntityHierarchy, ClientKey, … as in the column reference tables). The form can show more fields when the user picks a workspace that has OLS/RLS (e.g. show RLS dimensions only when Workspace = CDI/AMER/WFI/GI/DFI/EMEA).
- **One submit.** The user fills everything and clicks Submit once. The frontend sends all of that to the create API (one request body or one API call with the full payload).
- **How it connects to the DB:** The backend receives that single payload. It then writes in one go: **PermissionRequests** (from request fields + system values), **PermissionHeaders** (one or two rows: OLS and/or RLS), then **OLSPermissions** (if OLS was provided), **RLSPermissions** (if RLS was provided), and the correct **RLSPermission*Details** table (CDI, AMER, WFI, GI, FUM, or EMEA) with the dimension values the user entered. So all those table rows are created from the **one tabular form** and the **one API call**—no wizard steps, no separate pages or grids.

### Text-based representation: what the single flow looks like

One screen. All fields below appear on the same form; user fills them and clicks **Submit** once.

```
┌─────────────────────────────────────────────────────────────────────────┐
│  New permission request (tabular entry)                                  │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  Request                                                                  │
│  ─────────                                                                │
│  Workspace          [ Dropdown: CDI | AMER | WFI | GI | DFI | EMEA ]     │
│  RequestedFor       [ Lookup: employee ]                                 │
│  RequestedBy        [ Lookup or current user ]                          │
│  LMApprover         [ Lookup: employee ]                                 │
│  RequestReason      [ Text ________________________________ ]            │
│                                                                          │
│  (If OLS needed)                                                          │
│  ───────────────                                                          │
│  OLS item type      [ Dropdown: Report (SAR) | Audience ]                │
│  Report / Audience  [ Lookup: select report or audience ]                │
│  AdditionalDetails [ Optional JSON or structured fields ]               │
│                                                                          │
│  RLS (dimensions depend on Workspace – example for CDI)                  │
│  ───────────────────────────────────────────────────────                 │
│  SecurityType       [ Dropdown: Orga | PA | Client | CC | MSS | PC ]    │
│  EntityKey          [ Lookup / Dropdown ]    EntityHierarchy [ Dropdown ]│
│  ClientKey          [ Lookup / Dropdown ]    ClientHierarchy [ Dropdown ]│
│  SLKey              [ Lookup / Dropdown ]    SLHierarchy     [ Dropdown ]│
│  (For AMER: add SL, PC, CC, PA, MSS. For WFI: Entity + PA only.          │
│   For GI: Entity, Client, MSS, SL. For DFI: Entity, Country, Client,    │
│   MSS, ProfitCenter. For EMEA: Entity, Country, Client, CC, MSS, SL.)    │
│                                                                          │
│  AdditionalDetailsJSON (if needed, e.g. DFI) [ Text / structured ]       │
│                                                                          │
│                              [ Cancel ]    [ Submit ]                     │
└─────────────────────────────────────────────────────────────────────────┘
```

- **Request** and **RLS** (and **OLS** when used) are all on this one screen. No "Next" or "Previous"; no separate steps.
- After **Submit**, the app shows success and goes back to the list or refreshes. RequestCode appears after create (system-generated).
