...
- **Session:** There is **no server-side session store**. Session is **token-based** and **stateless**: the frontend holds the access token (and optionally a refresh token); the backend validates the token on every request and derives the user from it. See the **Session management** section for the full picture.
...
| **Session** | In this setup, **no server-side session store**. Session is **token-based** and **stateless**: the frontend holds the access token (and optionally a refresh token in a cookie); the backend validates the token on every request and derives the user (oid → User table). Session “ends” when the token is removed (logout) or expired and not refreshed. See **Session management** section. |
...
| *(Session)* | *(Not a phase)* | Session is **token-based** and **stateless** throughout: no server-side session store; see **Session management** in the Auth and roles section. |
...
**Goal:** Backend validates the Bearer token, resolves **oid** to the **User** table (stable link), and uses **UserId** + **current email** from DB for all authorization. No trust of client-supplied `forUser`. **Email change** = update one User row. The backend is **stateless** (no server-side session store); each request is authorized using the token only — see **Session management**.
...
**Goal:** Frontend never sends a fake `forUser`; it sends only the Bearer token; workspace list and audit fields use the current user from the backend. The frontend **holds the session** by storing the access token (e.g. in localStorage or memory) and optionally using a refresh token; there is no server-side session — see **Session management**.
...
- [ ] **Session:** No server-side session store; frontend holds access token and sends it on every request; optional refresh and logout revoke — see **Session management** section.

---

## Auth and roles: detailed flows (Mermaid)

**Diagram plan:** The following 10 diagrams explain the full auth and role model. Each is syntactically valid Mermaid.

| # | Diagram | Purpose |
|---|---------|---------|
| 1 | High-level auth flow | Login to first load: Entra → token → /auth/me → User table → what frontend stores. |
| 2 | Per-request authorization flow | Any API call: token → validate → oid → User → role checks → return only allowed data. |
| 3 | What frontend receives and verification | /auth/me response; frontend does not verify roles (backend enforces); frontend uses data for UI only. |
| 4 | Workspace Admin/Owner | Source: Workspaces CSV. Backend: GetWorkspacesForUserAsync(User.Email). What frontend gets. |
| 5 | Requester | No role table; backend filters by current user. What frontend gets. |
| 6 | Requester + Approver | Workspace list + approval queue from RLS/OLS. What frontend gets. |
| 7 | Support | SupportUsers table → all workspaces. What frontend gets. |
| 8 | Platform Admin | PlatformAdmins table → allow sensitive actions. What frontend gets; backend 403 if not. |
| 9 | Summary: all 5 user types | One view: source of truth, what backend returns, what frontend receives per type. |
| 10 | End-to-end: token to response | Single request path through backend (token → User → checks → response). |
| — | **Session management** | **Token-based, stateless session:** no server-side session store; frontend holds token; backend validates every request. Session lifecycle (login, each request, refresh, logout) — see below. |

---

### Session management (how “session” works in this setup)

In this setup there is **no traditional server-side session store** (no in-memory session id on the server). The “session” is **token-based**: the frontend holds an **access token** (and optionally a **refresh token**), and the backend is **stateless** — it validates the token on every request and derives the user from it (oid → User table). Here is how that behaves across the app and over time.

| Aspect | Where it lives | How it works |
|--------|----------------|--------------|
| **Who is logged in** | **Frontend:** access token in memory or localStorage (e.g. `sakura_jwt_token`). Optional: refresh token in an HttpOnly cookie (set by backend on login). | Every API call sends `Authorization: Bearer <access token>`. Backend does not keep a “session id”; it reads the token each time. |
| **User identity for the request** | **Backend:** not stored. Computed each request from the token (oid → User table → ICurrentUserService). | Validate token → extract oid → resolve to User row → use UserId/Email for authorization. No server-side “session” object. |
| **Session duration** | **Access token:** has an expiry (e.g. 1 hour). **Refresh token (if used):** longer-lived (e.g. 7 days), stored in DB and/or cookie. | While the access token is valid, the user stays “logged in” from the app’s perspective. After it expires, either the user must log in again or the frontend uses the refresh token to get a new access token (see below). |
| **Cross-tab / same browser** | **Frontend:** if the token is in **localStorage**, all tabs share it — one tab’s login is effectively the same “session” in other tabs. If the token is in **memory only**, each tab has its own; closing the tab “ends” that tab’s session. | Same backend behaviour in all cases: whoever sends a valid token is treated as that user. |
| **Logout** | **Frontend:** remove the access token (and clear any stored user). **Backend (if refresh token is used):** revoke the refresh token in DB and clear the refresh cookie. | After logout, the access token is no longer sent; even if it is still valid until expiry, the app does not use it. Revoking the refresh token prevents getting new access tokens. |

**Refresh flow (optional):** If the backend supports it, the frontend can call `POST /auth/refresh` (e.g. when the access token is about to expire). The backend reads the refresh token from the cookie (or body), checks it in the DB, and returns a new access token (and optionally a new refresh token). That keeps the user “in session” without re-login until the refresh token expires or is revoked.
...
sequenceDiagram
  ...
  Note over FE: Login
  U->>FE: Sign in (Entra or JWT)
  FE->>BE: POST /auth/login (or token from Entra)
  BE-->>FE: Access token (+ optional refresh cookie)
  FE->>FE: Store token (e.g. localStorage)

  Note over FE,BE: Each request (no server-side session)
  loop Every API call
    FE->>BE: Request + Bearer token
    BE->>BE: Validate token, resolve oid to User
    BE-->>FE: Response (data for that user only)
  end

  Note over FE: Token expired?
  alt Refresh supported
  ...
  Note over FE: Logout
  U->>FE: Logout
  FE->>FE: Clear token and stored user
  FE->>BE: POST /auth/logout (optional: revoke refresh)
  BE->>BE: Revoke refresh token in DB
```

**Summary:** Session is “I have a valid access token (and optionally a refresh token).” The backend does not track sessions; it only validates the token and resolves the user each time. Session “ends” when the token is removed (logout) or expired and not refreshed.

---

**Session and the whole doc:** The token-based, stateless session model above applies to every phase in this document — no server-side session store anywhere.

### 1. High-level auth flow (login to first load)
...
sequenceDiagram
  ...
  Entra->>FE: Redirect with token (oid, email)
  FE->>FE: Store token
  FE->>BE: GET /auth/me (Bearer token)
  BE->>BE: Validate token, extract oid
  BE->>DB: Lookup User by EntraObjectId
  alt User not found
    DB-->>BE: None
    BE->>DB: INSERT User (oid, email from token)
  else User found, email changed
    BE->>DB: UPDATE User.Email, UpdatedAt
  end
  DB-->>BE: User (UserId, Email)
  BE->>DB: Check SupportUsers, PlatformAdmins (optional)
  BE-->>FE: 200 { userId, email, name, isSupport?, isPlatformAdmin? }
  FE->>FE: Store in AuthService (currentUser$)
  FE->>U: Show app (menu, workspace list next)
```

---

### 2. Per-request authorization flow (e.g. GET /workspaces)
...
### 3. What frontend receives and verification

```mermaid
flowchart LR
  subgraph Backend["Backend (source of truth)"]
    ME["GET /auth/me"]
    ME --> R["Response: userId, email, name, isSupport?, isPlatformAdmin?"]
  end

  subgraph Frontend["Frontend (UI only)"]
    Store["Store in AuthService"]
    Display["Display: user name, email"]
    Menu["Menu: show Admin link if isPlatformAdmin"]
    Hide["Never trust for security: backend enforces on every API call"]
  end
  ...
  subgraph Rule["Rule"]
    T["Frontend cannot 'verify' roles: it only shows/hides UI. Backend returns only data the user is allowed to see."]
  end
```

**What frontend receives from /auth/me:** `{ userId, email, name, isSupport?, isPlatformAdmin? }` (or equivalent).  
**How frontend can "verify":** It does not verify for security. Backend enforces on every API call. Frontend uses the response only to show the right UI (e.g. admin menu, display name).  
**Required information:** Only what is needed for display and menu; no sensitive role data beyond what backend already restricts in API responses.
...
```mermaid
flowchart TB
  subgraph Source["Source of truth (DB)"]
    W[(Workspaces: WorkspaceOwner, WorkspaceTechOwner, WorkspaceApprover CSV)]
  end

  subgraph Backend["Backend"]
    U[ICurrentUserService.Email from User table]
    U --> Q["GetWorkspacesForUserAsync(email)"]
    Q --> M["Match: email in Owner or TechOwner or Approver CSV"]
    M --> L["Return list of workspaces"]
  end

  subgraph Frontend["Frontend receives"]
    LIST["Workspace list: only workspaces where user is Owner/TechOwner/Approver"]
    LIST --> UI["Show workspace dropdown / list; user can manage only these"]
  end
...
**Who:** User whose email is in `Workspaces.WorkspaceOwner`, `WorkspaceTechOwner`, or `WorkspaceApprover` for that workspace.  
**What frontend receives:** List of workspaces from GET /workspaces (backend already filtered).  
**Verification:** Backend only; frontend just displays the list. No extra "verify" call.
...
```mermaid
flowchart TB
  subgraph Source["No role table"]
    N["Any authenticated user (has User row)"]
  end

  subgraph Backend["Backend"]
    E[ICurrentUserService.UserId / Email]
    E --> CR["Create request: CreatedBy = current user"]
    E --> MR["My requests: filter by CreatedBy or UserId"]
    CR --> OK[200]
    MR --> LIST["Return only this user's requests"]
  end

  subgraph Frontend["Frontend receives"]
    F1["Can call: create request, my requests"]
    F2["List of requests they created"]
  end
...
**Who:** Any signed-in user. No separate table.  
**What frontend receives:** Ability to create requests; list of "my requests" (backend filters by current user).  
**Verification:** Backend filters; frontend only shows what backend returns.

---

### 6. Requester + Approver (same user)

```mermaid
flowchart TB
  subgraph Source["Source of truth (DB)"]
    WS[(Workspaces: Owner, TechOwner, Approver CSV)]
    RLS[(RLS/OLS approver tables)]
  end

  subgraph Backend["Backend"]
    Email[ICurrentUserService.Email]
    Email --> WQ["GetWorkspacesForUserAsync(email)"]
    WQ --> WL["Workspaces where user is Owner or Approver"]
    Email --> AQ["Pending approvals: where user in RLS/OLS approver list"]
    AQ --> AL["List of items awaiting their approval"]
  end

  subgraph Frontend["Frontend receives"]
    F1["Workspace list (same as Workspace Admin where they are owner/approver)"]
    F2["Pending approvals queue"]
    F3["Can approve/reject; can create requests"]
  end
...
**Who:** User in Workspace Approver CSV or in RLS/OLS approver tables for a workspace.  
**What frontend receives:** Workspace list (from GET /workspaces) + pending approvals (from approval API). Backend filters both by current user.  
**Verification:** Backend only; frontend displays what backend returns.
...
flowchart TB
  subgraph Source["Source of truth (DB)"]
    SU[(SupportUsers: UserId or EntraObjectId)]
  end

  subgraph Backend["Backend"]
    UID[ICurrentUserService.UserId]
    UID --> CHK{UserId in SupportUsers?}
    CHK -->|Yes| ALL["GetAnyWorkspacesAsync()"]
    CHK -->|No| NORM["GetWorkspacesForUserAsync(email)"]
    ALL --> FULL["Return ALL workspaces"]
  end

  subgraph Frontend["Frontend receives"]
    F1["Full workspace list"]
    F2["Can assist any workspace; see all"]
  end
...
**Who:** User with a row in `SupportUsers` (keyed by UserId or EntraObjectId).  
**What frontend receives:** Full workspace list from GET /workspaces; can open any workspace.  
**Verification:** Backend checks SupportUsers; frontend gets all workspaces only if backend says so.
...
flowchart TB
  subgraph Source["Source of truth (DB)"]
    PA[(PlatformAdmins: UserId or EntraObjectId)]
  end

  subgraph Backend["Backend"]
    UID[ICurrentUserService.UserId]
    UID --> CHK{UserId in PlatformAdmins?}
    CHK -->|Yes| ALLOW["Allow: create workspace, app settings, event logs"]
    CHK -->|No| DENY["403 Forbidden on those actions"]
  end

  subgraph Frontend["Frontend receives"]
    F1["Same /auth/me as others; isPlatformAdmin = true for UI"]
    F2["Admin menu visible; can call create workspace, settings"]
    F3["Backend returns 403 if not in PlatformAdmins"]
  end
...
**Who:** User with a row in `PlatformAdmins` (or in `UserRoles` with role PlatformAdministrator).  
**What frontend receives:** Same /auth/me; optional `isPlatformAdmin` for showing admin menu. Backend enforces: 403 if they call create workspace / settings and are not in PlatformAdmins.  
**Verification:** Backend only; frontend does not "verify" — it only shows/hides admin UI.

---

### 9. Summary: all 5 user types

```mermaid
flowchart TB
  subgraph Types["User type | Source of truth | What backend returns | What frontend receives"]
    T1["1. Workspace Admin/Owner | Workspaces CSV | Workspaces where email in Owner/TechOwner/Approver | Workspace list"]
    T2["2. Requester | No table | My requests, create request | Request list, create form"]
    T3["3. Requester/Approver | Workspaces CSV + RLS/OLS tables | Workspace list + pending approvals | Workspaces + approval queue"]
    T4["4. Support | SupportUsers table | ALL workspaces | Full workspace list"]
    T5["5. Platform Admin | PlatformAdmins table | Allow create WS, settings; 403 if not | Admin menu if isPlatformAdmin; 403 on action if not"]
  end

  subgraph Rule["Frontend never verifies roles for security; backend returns only allowed data"]
    R1["GET /auth/me: userId, email, name, isSupport?, isPlatformAdmin?"]
    R2["All other APIs: filtered by current user (UserId/Email from User table)"]
  end
```

| User type | Source of truth (DB) | What backend returns | What frontend receives |
|-----------|----------------------|------------------------|-------------------------|
| **1. Workspace Admin/Owner** | `Workspaces.WorkspaceOwner`, `WorkspaceTechOwner`, `WorkspaceApprover` (CSV) | List of workspaces where `User.Email` is in those columns | Workspace list; user manages only these |
| **2. Requester** | No role table | "My requests" filtered by current user; allow create request | Request list; create form |
| **3. Requester/Approver** | Same as 1 + RLS/OLS approver tables | Workspace list + pending approvals for this user | Workspaces + approval queue |
| **4. Support** | `SupportUsers` (UserId or EntraObjectId) | All workspaces | Full workspace list |
| **5. Platform Admin** | `PlatformAdmins` (UserId or EntraObjectId) | Allow create workspace, app settings, event logs; 403 if not in table | Same /auth/me; admin menu if isPlatformAdmin; 403 on protected actions if not admin |

---

### 10. End-to-end: token to response (one request)
...
| **Session: token-based, stateless** | No server-side session store; backend validates token every request | One validation per request | Scales horizontally | Same behaviour in all envs; see **Session management** |
...
4. **Frontend**  
   ...
   - **Session:** Store access token (e.g. localStorage); send `Authorization: Bearer <token>` on every API call. Optional: use refresh token (cookie) to get new access token before expiry. Logout: clear token and call `POST /auth/logout` to revoke refresh token. No server-side session — see **Session management** section.  
