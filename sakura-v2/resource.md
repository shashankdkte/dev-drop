...
**Asset type:** Azure PaaS (no traditional IaaS VMs in this application path)  
**Last updated:** 2026-09-11  
**Evidence:** Azure Portal JSON — SQL Dev/UAT; App Service Dev/UAT/Prod; Prod Key Vault
...
| Legacy Gapteq / V1 hosts on same Entra app | Out of Sakura V2 scope unless explicitly added |

---

## 9. Suggested reply text (copy for InfoSec)

> Please find the Sakura V2 host inventory for vulnerability assessment / Defender + Wiz coverage.  
> Scope is Azure PaaS only (Static Web Apps, App Service Linux .NET 8, Azure SQL, Key Vault) across Development, UAT, and Production — no application VMs.  
> APIs are public Linux App Services (`DOTNETCORE|8.0`): Dev inbound `20.105.232.38` / `20.105.224.52` (Owner Ebad.Uddin); UAT inbound `20.105.216.32` / `20.105.243.12` and Prod inbound `20.105.216.55` / `20.105.224.134` (Owner Toniann.Tuson).  
> Prod Key Vault `azeuw1pkvsakura` (`azeuw1pkvsakura.vault.azure.net`) is in `AZ-VDC000007-EUW1-RG-BI-PROD-CENTRAL`; Owner Toniann.Tuson; network ACL default Allow (no IP/VNet rules) — please validate exposure.  
> Dev/UAT SQL remain in Sensei RGs with Owner patrick.sura@dentsu.com and `publicNetworkAccess=Enabled`. Prod SQL portal export still outstanding.
